...
namespace MaddenEditor.Core
{
	public class CoachEditingModel
	{
        ...
        public void InitCoachList()
        {
            ...
            if (FilterCoachTeam == -2)
            {
                if (config.streamdb_model == null)
                    return;
                foreach (CoachCollection coll in config.streamdb_model.TableModels[EditorModel.COACH_COLLECTIONS_TABLE].GetRecords())
                {
                    if (coll.Deleted)
                        continue;
                    else if (!coachlist.Contains(coll.CoachId))
                ...
                {
                    TableRecordModel rec = config.streamdb_model.TableModels[EditorModel.COACH_COLLECTIONS_TABLE].GetRecord(i);
                    CoachCollection coachcoll = (CoachCollection)rec;
                    if (!CoachNames.ContainsKey(coachcoll.CoachId))
        ...
        public CoachRecord CurrentCoachRecord
		{
			...
			get
			{
                if (currentstreamedID != -1 && config.streamdb_model != null)
                {
                    CoachCollection coll = (CoachCollection)config.streamdb_model.TableModels[EditorModel.COACH_COLLECTIONS_TABLE].GetRecord(currentstreamedID);
        ...
        public CoachRecord GetCoachById(int id)
        {
            ...
            if (config.streamdb_model != null)
            {               
                foreach (TableRecordModel rec in config.streamdb_model.TableModels[EditorModel.COACH_COLLECTIONS_TABLE].GetRecords())
                {
                    if (rec.Deleted)
                        continue;
        ...
        {
            bool exists = false;
            if (newteam == 1023)    // free agents no need to check
                return false; 
